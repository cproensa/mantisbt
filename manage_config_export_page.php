...
foreach( $t_providers as $t_id => $t_provider ) {
	...
	if( isset( $t_config_rows[$t_id] ) ) {
		...
		# if does not exist in config, it's enabled by default
		$t_row['enabled'] = true;
	}
	if( $t_row['enabled'] ) {
...
function print_export_row( $p_row, $p_type ) {
	...
	switch( $p_type ) {
		case 'ENABLED':
			...
			print_form_button( $t_submit_page, 'DISBALE', $t_params);
			break;
		case 'DISABLED':
...
//			<form id="manage-columns-form" method="post" action="manage_config_export_set.php">
